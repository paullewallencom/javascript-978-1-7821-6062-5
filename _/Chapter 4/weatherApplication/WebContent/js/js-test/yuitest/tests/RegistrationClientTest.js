...
YUI().use('test', 'console', function(Y) {
	var emptyFieldsTestcase = new Y.Test.Case({
		...
		testEmptyPassword: function() {
	 		...
	 		Y.Assert.areEqual(document.getElementById("passwordMessage1").innerHTML, "(field is required)");	
		}
	});
	...
	var matchedPasswordsTestcase = new Y.Test.Case({
		...
		testMatchedPasswords: function() {
	 		...
	 		Y.Assert.areEqual(document.getElementById("passwordMessage1").innerHTML, "(Passwords must be identical)");	
		}
	});	
	...
	var fieldsFormatTestcase = new Y.Test.Case({
		...
		testPasswordFormat: function() {
	 		...
	 		Y.Assert.areEqual(document.getElementById("passwordMessage1").innerHTML, "(format is invalid)");	
		}		
	});		
	...
	var registerUserTestcase = new Y.Test.Case({
		...
		testAddNewUser: function() {
			...
			var successCallback = function(response) {
				...
				this_local.resume(function() {
					Y_local.Assert.areEqual(resultMessage, "User is registered successfully ...");
				});
			};
		...
		testAddExistingUser: function() {
			...
			var failureCallback = function(response) {
				var resultMessage = response.xmlhttp.responseText;
				this_local.resume(function() {
					Y_local.Assert.areEqual(resultMessage, "A user with the same username is already registered ...");								
				});							
			};					
